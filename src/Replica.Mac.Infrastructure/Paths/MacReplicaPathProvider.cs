...
namespace Replica.Mac.Infrastructure.Paths;

public sealed class MacReplicaPathProvider : IReplicaPathProvider
{
    public MacReplicaPathProvider()
        : this(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData))
    {
    }

    public MacReplicaPathProvider(string localApplicationDataDirectory)
    {
