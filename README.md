...
Replica now has a compilable .NET 10 WPF desktop bootstrap, a versioned and hardened `.replica` snapshot format, and a read-only Windows environment scanner for applications, winget and Store identities, environment variables, PATH entries, Windows metadata, fonts, and built-in plugins. Software installation, settings restoration, and every system-changing capability remain unimplemented. The staged delivery plan is in [ROADMAP.md](docs/ROADMAP.md).
