...
Replica now has a compilable .NET 10 WPF desktop bootstrap and a versioned, checksummed `.replica` snapshot format with atomic writing, hardened reading, explicit file selection, bounded resource validation, and optional authenticated encryption. Machine scanning, software installation, settings restoration, and every system-changing capability remain unimplemented. The staged delivery plan is in [ROADMAP.md](docs/ROADMAP.md).
