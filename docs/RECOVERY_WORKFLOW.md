...
4. **Select data:** The user explicitly chooses folders and files. For an Offline Pack, each installer additionally needs provenance review and a Windows-trusted Authenticode publisher; Replica records its publisher-certificate SHA-256, file SHA-256, size, version, architecture, and licensing warning. Replica estimates size, flags risky selections, and records exclusions.
...
10. For an Offline Pack, optionally export verified installers to a separately approved folder. Replica rechecks the pack and publisher-bound installer identity, refuses overwrite, and never executes the files.
11. Rescan affected areas and display remaining differences, restart requirements, and manual follow-up.

The WPF implementation persists 19 recovery safety steps but presents them as five understandable phases. It supports explicit drive mapping, hardware-dependent warnings, bounded checksum-protected resume state, failed-action-only retries, and before/after similarity. See [Post-reset recovery wizard](RECOVERY_WIZARD.md).
